...
const rgbToHex = (rgb: number[]) => `#${rgb.map(val => val.toString(16))}`;

function mixColors(a: string, b: string, output: number) {
    const colorA = strToRgb(a);
    const colorB = strToRgb(b);

    return new Array(output).fill(0).map((_, i) => {
        const n = i / (output - 1);
